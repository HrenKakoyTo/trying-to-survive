...
int start_menu::paint(vector<int> &ctrl_data) {
    ...
    if(IntRect(100, 350, 130, 17).contains(ctrl_data[1], ctrl_data[2])){
        return_value =  2;
    }
    if(IntRect(50, 50, 50, 50).contains(ctrl_data[1], ctrl_data[2])){
        ctrl_data[4] = 0; //если будут другие карты, то мы допишем
    }
    if(IntRect(120, 50, 50, 50).contains(ctrl_data[1], ctrl_data[2])){
        ctrl_data[3] = 0; //если будут другие карты, то мы допишем
    }
